...
import { Copy, Check, ArrowRight, Play, CircleAlert, HardDrive, FileSearch, Trash2 } from "lucide-react";
      ...
      <main className="flex flex-col scroll-smooth" role="main">
          ...
          <div className="container max-w-4xl mx-auto">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="w-full grid grid-cols-2 md:grid-cols-4 h-auto">
                ...
                <TabsTrigger value="near">Near-Duplicate</TabsTrigger>
              ...
              {[
                {
                  ...
                  description: "Run a library scan and the Exact Duplicate Detector immediately groups all three packages together with a 100% match score. Smart Keep Recommendations identify which copy has the best license type and file count. Remove the two redundant copies to your holding folder and recover gigabytes of disk space — all in under two minutes.",
                },
                {
                  icon: FileSearch,
                  title: "Finding overlapping articles before publishing to multiple sites",
                  description: "Before assigning PLR articles across your network of niche sites, run the Content Overlap Scanner to identify any articles that appear in multiple packages. Publishing the same article on two different sites creates a duplicate content risk with Google. The scanner catches these overlaps before they cost you rankings.",
                },
                {
                  icon: Trash2,
                  title: "Cleaning up a 500-package PLR library bought over several years",
                  description: "Point PLR Organizer Pro at your entire PLR folder and let the duplicate scan run. For a library of 500 packages, the scan flags every exact, near, and overlapping duplicate group automatically. Review each group at your own pace using the Duplicates sidebar view, accept the Smart Keep Recommendations, and remove redundant packages safely — with full undo available throughout.",
                },
              ].map((card, i) => (
        ...
        <section className="py-16 bg-gradient-primary text-primary-foreground" aria-label="Call to Action">
          <div className="container max-w-3xl mx-auto text-center">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">Ready to Use Duplicate & Overlap Detector?</h2>
            <p className="text-lg opacity-90 mb-8">Get instant access to Duplicate & Overlap Detector and all 3 other powerful tools included with PLR Organizer Pro.</p>
            <Link to="/funnel/offer">
